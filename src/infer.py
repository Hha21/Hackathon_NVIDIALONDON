...
def run_inference(args):
    ...
    for s_idx, stn_name in enumerate(stations):
        ...
        # Batch all rollouts for this station into one generate() call: (n_rollouts, T).
        # generate() is batch-agnostic, so this saturates the GPU instead of issuing
        # n_rollouts serial batch-1 calls (~50x fewer kernel launches → seconds not minutes).
        prompt = (
            torch.tensor(seed, dtype=torch.long)
            .unsqueeze(0)
            .repeat(args.n_rollouts, 1)
            .to(device)
        )
        ...
        for r in range(out.size(0)):
            tokens    = [inv_vocab.get(int(t), "?") for t in out[r].tolist()]
