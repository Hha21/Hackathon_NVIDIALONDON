...
(function () {
  ...
  function riskColor(v) {
    ...
    for (let i = 0; i < stops.length - 1; i++) if (v >= stops[i][0] && v <= stops[i + 1][0]) {
      a = stops[i];
      b = stops[i + 1];
      break;
    }
  ...
  function add3DBuildings(map, minzoom, holo, hMult) {
    try {
      ...
      const color = holo ? ["interpolate", ["linear"], ["coalesce", ["get", "render_height"], 10], 0, "#15181e", 60, "#1d222a", 160, "#262c35"] : ["interpolate", ["linear"], ["coalesce", ["get", "render_height"], 10], 0, "#46506a", 25, "#5a6684", 80, "#7a6a9a", 180, "#9a6a72"];
      map.addLayer({
        id: "fd-3d",
        source: vid,
      ...
      console.log("3D buildings fail: " + e);
    }
  }
    ...
    fetch("https://overpass-api.de/api/interpreter", {
      ...
      (j.elements || []).forEach(w => {
        ...
        for (let i = 1; i < g.length; i += 3) {
          if (near(g[i].lon, g[i].lat)) feats.push({
            type: "Feature",
            geometry: {
              type: "Point",
              coordinates: [g[i].lon, g[i].lat]
            },
            properties: {}
          });
        }
  ...
  function addHydrants(map) {
    ...
    map.on("moveend", () => {
      clearTimeout(t);
      t = setTimeout(() => {
        if (map.getZoom() >= 12.5) fetchHydrants(map);
      }, 700);
    });
  }
  ...
  function addRoute(map, you) {
    ...
    draw([you, BROCKLEY]);
    fetch(`https://router.project-osrm.org/route/v1/driving/${you[0]},${you[1]};${BROCKLEY[0]},${BROCKLEY[1]}?overview=full&geometries=geojson`).then(r => r.json()).then(j => {
      const c = j && j.routes && j.routes[0] && j.routes[0].geometry && j.routes[0].geometry.coordinates;
      if (c && c.length) draw(c);
  ...
  function addHotspots(map) {
    ...
    WARDS.forEach(w => {
      ...
      el.innerHTML = '<div class="wm-card"><div class="wm-top"><span class="wm-dot"></span>' + '<span class="wm-name">' + w.name + '</span><span class="wm-risk"></span></div>' + '<div class="wm-sub">' + w.type + '</div></div><div class="wm-stem"></div>';
      new mgl.Marker({
        element: el,
        anchor: "bottom"
      }).setLngLat([w.lon, w.lat]).addTo(map);
      reg[w.name] = {
        dot: el.querySelector(".wm-dot"),
        risk: el.querySelector(".wm-risk")
      };
    });
    const paint = (name, v) => {
      const r = reg[name];
      ...
      r.risk.textContent = v.toFixed(2);
      r.risk.style.color = c;
    };
    WARDS.forEach(w => paint(w.name, w.base));
    // Keep the on-map markers in sync with the time slider (so map == ranked list).
    window.__globeUpdate = risks => {
      Object.keys(risks).forEach(n => paint(n, risks[n]));
    };
    window.__globeFocus = (lon, lat) => {
      try {
        map.flyTo({
          center: [lon, lat],
          zoom: 14.5,
  ...
  function Map3D({
    ...
    React.useEffect(() => {
      ...
      map.on("load", () => {
        ...
        try {
          if (map.getLayer("background")) map.setPaintProperty("background", "background-color", isHeat ? "#0A0B0D" : "rgba(0,0,0,0)");
        } catch (e) {}
        ...
        if (isHeat) {
          addHotspots(map);
          addHydrants(map);
        } // hydrants (200m of wards) only on the globe
      ...
      const tid = setTimeout(() => {
        try {
          map.resize();
          console.log("MAPRESIZE " + map.getContainer().clientWidth + "x" + map.getContainer().clientHeight);
        } catch (e) {}
      }, 600);
      return () => {
        clearTimeout(tid);
        ...
        if (isHeat) window.__globeFocus = null;
      };
    }, [preset]);
    return /*#__PURE__*/React.createElement("div", {
      ref: ref,
      className: "mlmap " + className,
      style: style
    });
  }
  window.Map3D = Map3D;
